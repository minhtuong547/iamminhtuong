...
$code = $soicoder->real_escape_string(check_string($_POST['code']));
...
if ($his == 0) {
    ...
    if ($data['status'] == 'off') {
        ...
        $return = array(
            'status' => 'error',
            'msg' => "Lỗi Do Zalopay Trả Thưởng Không Tồn Tại"
        );
        die(json_encode($return));
        ...
        if ($select == 0) {
            $return = array(
                'status' => 'error',
                'msg' => "Số Zalopay Không Tồn Tại Trên Hệ Thống"
            );
            die(json_encode($return));
        } else {
            ...
            if ($send['status'] == 'success') {
                ...
                $return = array(
                    'status' => 'error',
                    'msg' => "Lỗi Zalopay, Vui Lòng Liên Hệ Admin"
                );
                die(json_encode($return));
            }
        }
