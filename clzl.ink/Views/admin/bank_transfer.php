            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        ...
                        Chuyển Tiền Về Ngân Hàng
                    </h3>
                </div>
                <div class="card-body">
                    <form action="/api/zalopay/send_bank" method="post">
                        ...
                        <div class="form-group">
                            <label for="" class="form-label">Ngân Hàng</label>
                            <select class="form-control" name="bank" id="bank">
                                <option>Chọn ngân hàng</option>
                                <?php
                                $list = $soicoder->fetch_assoc("SELECT * FROM `code_bank` ORDER BY id desc", 0);
                                foreach ($list as $data) { ?>
                                    <option value="<?= $data['bcbankcode']; ?>-<?= $data['bankcode']; ?>-<?= $data['fullname']; ?>-<?= $data['name']; ?>"><?= $data['name']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="" class="form-label">Số Tài Khoản</label>
                            <input type="text" name="stk" id="stk" onkeyup="getName()" class="form-control" placeholder="Nhập số tài khoản người nhận">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="amount" class="form-control" placeholder="Nhập số tiền cần chuyển">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-label">Nội Dung</label>
                            <input type="text" name="comment" class="form-control" placeholder="Nhập nội dung cần chuyển">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-label">Mật Khẩu Zalopay</label>
                            <input type="text" name="password" class="form-control" placeholder="Nhập mật khẩu zalopay">
                        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive table-mousewheel mb-3">
                        <table
                            ...
                            <tbody>
                            <?php
                                $history = $soicoder->fetch_assoc("SELECT * FROM `chuyen_tien` WHERE `type_gd` = 'sendbank' ORDER BY id desc", 0);
                                if (count($list) == 0) { ?>
                                    <tr>
                                        <td colspan="24">
                                            <div class="text-center">
    ...
    function getName() {
        $.ajax({
            ...
            success: function(data) {
                $("#name").attr('value', data.msg);
            },
            error: function() {
                $("#name").attr('value', "Lỗi Hệ Thống");
            }
        });
    }
</script>
